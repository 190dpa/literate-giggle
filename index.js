...
async function startServer() {
  ...
  bot.on('interactionCreate', async interaction => {
    try {
      ...
      // Comando /castigar
        ...
        const judgmentChannel = await interaction.guild.channels.create({
          ...
          permissionOverwrites: [
            ...
            { id: userToPunish.id, allow: ['ViewChannel', 'ReadMessageHistory'] }, // Permite para o usuário
        ...
        const judgmentRow = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`accept_punishment_${userToPunish.id}`).setLabel('Aceitar Castigo').setStyle(ButtonStyle.Success),
          new ButtonBuilder().setCustomId(`refuse_punishment_${userToPunish.id}`).setLabel('Recusar Castigo').setStyle(ButtonStyle.Danger)
        );

        await judgmentChannel.send({
          content: `<@${userToPunish.id}>, você foi castigado.`,
          embeds: [{
            title: '⚖️ JULGAMENTO',
            description: `Você foi colocado em julgamento pelo seguinte motivo:`,
            fields: [{ name: 'Motivo', value: reason }],
            color: 0xef4444,
            footer: { text: 'Aceite o castigo para ser liberado ou recuse para ser banido.' }
          }],
      ...
      else if (interaction.isButton() && interaction.customId.startsWith('accept_punishment_')) {
        const userId = interaction.customId.split('_')[2];
        if (interaction.user.id !== userId) return interaction.reply({ content: 'Você não pode interagir com o julgamento de outra pessoa.', ephemeral: true });

        await interaction.deferUpdate();
        await interaction.channel.send('Você aceitou seu castigo. O administrador foi notificado e irá liberá-lo em breve. O canal será deletado em 10 segundos.');
        
        // Aqui você pode notificar o admin, mas por enquanto vamos apenas deletar o canal.
        setTimeout(() => interaction.channel.delete('Castigo aceito.'), 10000);
      }

      else if (interaction.isButton() && interaction.customId.startsWith('refuse_punishment_')) {
        const userId = interaction.customId.split('_')[2];
        if (interaction.user.id !== userId) return interaction.reply({ content: 'Você não pode interagir com o julgamento de outra pessoa.', ephemeral: true });

        await interaction.deferUpdate();
        const member = await interaction.guild.members.fetch(userId);
        await interaction.channel.send('Você recusou o castigo e será banido do servidor.');
        
        await member.ban({ reason: 'Recusou o castigo durante o julgamento.' });
        setTimeout(() => interaction.channel.delete('Usuário banido após recusar castigo.'), 5000);
      }
